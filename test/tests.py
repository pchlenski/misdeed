...
def test_1():
    ...
    gen1 = OmicsGenerator(200, nodes=["mgx", "mbx"], node_sizes=[30, 30])

    gen1.add_interaction('F', "mbx", "mgx",  0.5-np.random.rand(30, 30), lag=1)
    f = gen1.get('F')
    print(f)
...
def test_2():
    ...
    n_mgx = 30
    n_mbx = 30
    gen2 = OmicsGenerator(200, nodes=["mgx", "mbx"], node_sizes=[30, 30])

    gen2.add_intervention('B', "mgx", 1*(0.5-np.random.rand(30)), affects_abundance=False, start=40, end=80)
    print(gen2)
...
def test_3():
    ...
    n_mgx = 30
    n_mbx = 30
    gen3 = OmicsGenerator(200, nodes=["mgx", "mbx"], node_sizes=[n_mgx, n_mbx])

    mgx = gen.get("mgx")
    mgx.log_noise = False

    gen3.add_interaction( 'A', "mgx", "mgx", 0.5-np.random.rand(n_mgx, n_mgx))
    gen3.add_intervention('B', "mgx",        1*(0.5-np.random.rand(n_mgx)), affects_abundance=False, start=40, end=80)
    gen3.add_intervention('C', "mgx",        1*(0.5-np.random.rand(n_mgx)), affects_abundance=True,  start=120, end=160)

    gen3.add_interaction('D', "mbx", "mbx",  0.5-np.random.rand(n_mbx, n_mbx))
    gen3.add_interaction('E', "mgx", "mbx",  0.5-np.random.rand(n_mgx, n_mbx))
    gen3.add_interaction('F', "mbx", "mgx",  0.5-np.random.rand(n_mbx, n_mgx), lag=1)
    f = gen3.get('F')
    print(f)
    print('inbound node', f.inbound_node)
    print('outbound node', f.outbound_node)
...
def test_4(n_nodes : int) -> None:
    ...
    for i in range(n_nodes):
        nodes.append(f"n{i}")
    test_gen = OmicsGenerator(100, nodes=nodes, node_sizes=np.random.randint(0, 999, size=n_nodes), silent=True)
    print(test_gen)
    print('***************************')
...
def test_5(
    ...
    gen1 = OmicsGenerator(t, ['a', 'b'], [a_size, b_size], init_full=True, silent=True)
...
def test_6():
    ...
    gen = OmicsGenerator(20, ['a', 'b'], [10, 10], init_full=True, silent=True)
    x,y,z = gen.generate()
    gen.save(x)
